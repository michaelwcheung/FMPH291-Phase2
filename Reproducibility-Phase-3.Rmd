...
df_1 <- select(df_study, all_of(outcome), all_of(exposure), all_of(covariates), "weights", "psu", "strata")
```
...
#table 1 structure
table1_names <- c(exposure, all_of(covariates))


tab1.1.cg <- compareGroups(~. - disc, data = select(df_1, all_of(table1_names), all_of(outcome)))
...
```{r Table 2}
#table 1 structure
tab2.cg <- compareGroups(any_ipv ~., data = select(df_1, all_of(covariates), exposure))
...
df_1 <- df_1 %>%
  mutate(PS = PropScore,
         weights = ifelse(any_ipv == 1, 
                          prop_exposed/PS, 
                          (1-prop_exposed)/(1-PS)))

#truncate weights at 5th and 95th percentiles
cutoff_5th <- quantile(df_1$weights, probs = .05)
cutoff_95th <- quantile(df_1$weights, probs = .95)

df_1 <- df_1 %>%
  mutate(trunc_weights = case_when(weights < cutoff_5th ~ cutoff_5th,
                                   weights > cutoff_95th ~ cutoff_95th,
                                   weights >= cutoff_5th && weights <= cutoff_95th ~ weights))
